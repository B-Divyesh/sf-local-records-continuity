...
assert.equal(build.headers.get("x-continuity-api-build"), "local-records-continuity-repair-9");
...
assert.ok([anonymous, reservedHeader, productHeader]
  .every((response) => response.headers.get("x-continuity-api-build") === "local-records-continuity-repair-9"));
...
console.log(JSON.stringify({ endpoint, csp: true, demo: true, notFound: 404, apiBuild: "local-records-continuity-repair-9", auth: { anonymous: 401, reservedHeader: 401, productHeader: 403 }, requests: responses.length, counts }));
